...
\begin{itemize}
	...
	\item Recurse into Model References -- This check-box will apply all changes chosen in Sections~\ref{lbl:leftside}, and \ref{lbl:rightside} inside of any \href{https://de.mathworks.com/help/simulink/model-reference.html}{referenced models}. The referenced models are transformed in memory and left open and unsaved: save each of them explicitly (or use \cmd{save\_system(model, 'SaveDirtyReferencedModels', 'on')}) if the transformed versions shall be kept, or close them without saving to keep the originals.
...
\begin{itemize}
	...
	\item Constants -- Reset the values of all \href{https://www.mathworks.com/help/simulink/slref/constant.html}{constants} (numbers, variables, and expressions) to \cmd{-17}. This odd value is chosen on purpose: a reset to 0 or 1 often lands on the value the model already had and would change nothing. A constant that already is \cmd{-17} becomes \cmd{-19}. The same rule applies to numeric Dialog Parameters.
	\item Functions -- Replace the code of all \href{https://www.mathworks.com/help/simulink/ug/what-is-a-matlab-function-block.html}{MATLAB function blocks} by a stub with the same inputs and outputs that sets every output to \cmd{-17}. Inside of a function block, arbitrarily complex functions may be hidden.
	...
	\item Stateflow -- These options rename the various named \href{https://www.mathworks.com/help/stateflow/ug/overview-of-stateflow-objects.html}{Stateflow elements} to have generic names. Currently charts, all chart data (inputs, outputs, local data, parameters, constants), messages, events, boxes, states, and functions are renamed. Transition labels are shortened to unrecognizable stubs.
\end{itemize}
...
\end{figure}
	...
	\item Signal Names -- This option removes the names of all signal lines and turns off \href{https://www.mathworks.com/help/simulink/ug/signal-label-propagation.html}{signal propagation}.
	...
	\item Block Sizes -- All block sizes and shapes are reset to their default values.
...
%---------------------------------------
\subsection{Elements \toolname Could Not Change}
\Simulink refuses some changes, e.g., on blocks inside locked library links, in read-only subsystems, on blocks that cannot be resized, or for parameters whose reset would add or remove ports. \toolname never aborts because of such an element: it skips it, continues with the next one, and prints a summary in the \Matlab Command Window at the end. The command \cmd{t = smokeLog('report')} returns a table with one row per skipped element (transformation, element path, parameter, and the \Simulink error). All elements listed there are still in their original state and must be inspected by hand before the model is shared.

\subsection{Scripting}
\toolname can also be called from scripts, e.g., to anonymize many models in batch mode: \cmd{SMOKE(model)} applies all transformations that are pre-selected in the GUI; \cmd{SMOKE(model, 'renameblocks', 0)} all but the given ones; \cmd{SMOKE(model, 'all', 0, 'removeannotations', 1)} only the given ones. See \cmd{help SMOKE} for all option names.
