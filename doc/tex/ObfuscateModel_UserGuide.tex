...
\toolname can also be called from scripts, e.g., to anonymize many models in batch mode: \cmd{SMOKE(model)} applies all transformations that are pre-selected in the GUI; \cmd{SMOKE(model, 'removeannotations', 1, 'renameblocks', 1)} applies only the given ones. See \cmd{help SMOKE} for all option names.
